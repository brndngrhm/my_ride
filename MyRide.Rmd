...
renderHighchart({
  ...
  validate(need(input$file_upload != "", "Upload a FIT file - This app displays power, heart rate, cadence, or speed of your ride. If any intervals are detected, this app also displays the average power, heart rate, cadence, or speed in these intervals."))
  ...
  hchart(changes(), "line", hcaes(x = datetime_to_timestamp(time), y=round(metric_avg,0)), color = "#A6E22E",showInLegend = F, name = input$measure_select) %>%
    ...
    hc_subtitle(text = if(input$measure_select == "Distance"){paste("Total Distance:", max(round(changes()$avg,0)), sep=" ")}
                else{paste("Overall Mean:", round(mean(changes()$metric_avg),2), sep=" ")},
                align = "left") %>%
