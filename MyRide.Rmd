...
plot_data <- reactive({
  ...
  rides_data_new[is.nan(rides_data_new)] <- 0
    ...
    validate(
      need(sum(rides_data_new$avg, na.rm = T) > 0, paste("There's no", input$measure_select, "data to plot :( Please select another option on the sidebar", sep=" "))
    )
  ...
  left_join(rides_data_new, changepoint_mean, by = "id" )
...
renderHighchart({
  ...
  #Main Plot 
  hchart(plot_data(), "line", hcaes(x=ride_min, y=round(avg,0)), color = "#A6E22E",showInLegend = F, name = input$measure_select) %>%
    hc_add_series(plot_data(), "line", hcaes(x=cumsum(count), y=round(mean)), color = "#1E90FF",  showInLegend = T, name = "Region Average") %>%
...
renderHighchart({
    ...
    validate(
      need(sum(hist_data()$value, na.rm = T) > 0, paste("There's no", input$measure_select, "data to plot :( Please select another option on the sidebar", sep=" "))
    )
