...
plot_data <- reactive({
  ...
  rides_data_new[is.nan(rides_data_new)] <- 0
    ...
    validate(
      need(sum(rides_data_new$avg) > 0, paste("There's no", input$measure_select, "data to plot :( Please select another option on the sidebar", sep=" "))
    )
...
renderHighchart({
    ...
    validate(
      need(sum(hist_data()$value) > 0, paste("There's no", input$measure_select, "data to plot :( Please select another option on the sidebar", sep=" "))
    )
